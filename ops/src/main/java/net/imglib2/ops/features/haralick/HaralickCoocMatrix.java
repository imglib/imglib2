...
public class HaralickCoocMatrix<T extends RealType<T>> extends AbstractFeature<CooccurrenceMatrix> {
    ...
    protected CooccurrenceMatrix recompute() {
        ...
        while (cursor.hasNext()) {
            cursor.fwd();
            pixels[cursor.getIntPosition(1) - (int)input.min(1)][cursor.getIntPosition(0) - (int)input.min(0)] =
                    (int)(((cursor.get().getRealDouble() - localMin) / (localMax - localMin + 1)) * nrGrayLevels);
        ...
        for (int y = 0; y < pixels.length; y++) {
            for (int x = 0; x < pixels[y].length; x++) {
                ...
                if (tx >= 0 && ty >= 0 && ty < pixels.length && tx < pixels[ty].length
