...
public class RightJoinedUnaryOperation< A, B > implements UnaryOutputOperation< A, B >
{
	...
	public B compute( A input, B output )
	{
		...
		{
			return Operations.concat( ( PipedUnaryOperation< B > ) first, ( UnaryOutputOperation< B, B > ) follower ).compute( ( B ) input, output );
		}
		else
		{
			return follower.compute( first.compute( input, first.bufferFactory().instantiate( input ) ), output );
		}
