...
public class And< R extends RealType<R> > extends BinaryOperation<R>
{
	public And(final Image<? extends RealType<?>> left, final Image<? extends RealType<?>> right) {
		super(left, right);
	}

	public And(final Operation<R> op, final Image<? extends RealType<?>> right) {
		super(op, right);
	}

	public And(final Image<? extends RealType<?>> left, final Operation<R> op) {
		super(left, op);
	}

	public And(final Operation<R> op1, final Operation<R> op2) {
		super(op1, op2);
	}
	...
	public And(final Operation<R> left, final Number val) {
		super(left, val);
	}

	public And(final Number val,final Operation<R> right) {
		super(val, right);
	}
