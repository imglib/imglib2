...
public class ToDegrees< R extends RealType<R> > extends UnaryOperation<R>
{
	public ToDegrees(final Image<? extends RealType<?>> img) {
		super(img);
	}

	public ToDegrees(final Operation<R> op) {
		super(op);
	}
	...
	public final void compute( final RealType<?> input1, final RealType<?> ignored, final R output ) {
		output.setReal(Math.toDegrees(input1.getRealDouble()));
	}
}