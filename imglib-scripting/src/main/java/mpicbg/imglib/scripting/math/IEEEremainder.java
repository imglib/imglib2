...
public class IEEEremainder< R extends RealType<R> > extends BinaryOperation<R>
{
	...
	public IEEEremainder(final Image<? extends RealType<?>> left, final Operation<R> op) {
		super(left, op);
	}

	public IEEEremainder(final Operation<R> op1, final Operation<R> op2) {
		super(op1, op2);
	}
	...
	public IEEEremainder(final Operation<R> left, final Number val) {
		super(left, val);
	}

	public IEEEremainder(final Number val,final Operation<R> right) {
		super(val, right);
	}
	...
	public final void compute( final RealType<?> input1, final RealType<?> input2, final R output ) {
		output.setReal(Math.IEEEremainder(input1.getRealDouble(), input2.getRealDouble()));
	}
}