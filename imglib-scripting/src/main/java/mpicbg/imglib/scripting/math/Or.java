...
public class Or< R extends RealType<R> > extends BinaryOperation<R>
{
	public Or(final Image<? extends RealType<?>> left, final Image<? extends RealType<?>> right) {
		super(left, right);
	}

	public Or(final Operation<R> op, final Image<? extends RealType<?>> right) {
		super(op, right);
	}

	public Or(final Image<? extends RealType<?>> left, final Operation<R> op) {
		super(left, op);
	}

	public Or(final Operation<R> op1, final Operation<R> op2) {
		super(op1, op2);
	}
	...
	public Or(final Operation<R> left, final Number val) {
		super(left, val);
	}

	public Or(final Number val,final Operation<R> right) {
		super(val, right);
	}
