...
/**
 * Implementation of Coarseness Tamura feature done by Marko Keuschnig &
 * Christian Penz as used in jfeaturelib
 * 
 */
public class Coarseness extends AbstractFeatureModule
{
	...
	private final int sizeLeadDiffValue( final int x, final int y )
	{
		...
		for ( int k = 0; k < 3; k++ )
		{
			tmp = Math.max( this.differencesBetweenNeighborhoodsHorizontal( x, y, k ), this.differencesBetweenNeighborhoodsVertical( x, y, k ) );
			if ( result < tmp )
			{
				maxK = k;
				result = tmp;
	...
	{
		double result = 0;
		result = Math.abs( this.averageOverNeighborhoods( x, y + ( int ) Math.pow( 2, k - 1 ), k ) - this.averageOverNeighborhoods( x, y - ( int ) Math.pow( 2, k - 1 ), k ) );
		return result;
	}
	...
	{
		double result = 0;
		result = Math.abs( this.averageOverNeighborhoods( x + ( int ) Math.pow( 2, k - 1 ), y, k ) - this.averageOverNeighborhoods( x - ( int ) Math.pow( 2, k - 1 ), y, k ) );
		return result;
	}
