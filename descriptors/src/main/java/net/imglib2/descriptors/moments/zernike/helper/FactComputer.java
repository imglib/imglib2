package net.imglib2.descriptors.moments.zernike.helper;
...
public class FactComputer
{
	...
	private boolean isPrime( final int n )
	{
		...
		{
			if ( ( n % m_primes.get( i ) ) == 0 ) { return false; }
		}
		return true;
	}
	...
	{
		int n = nParam;
		if ( ( nParam < 0 ) || ( nParam > m_high ) ) { throw new IllegalArgumentException( "Multiplication with out of range number!" ); }
		for ( int i = 0; i < m_primes.size(); ++i )
		{
	...
	{
		int n = nParam;
		if ( ( nParam < 0 ) || ( nParam > m_high ) ) { throw new IllegalArgumentException( "Division with out of range number!" ); }
		for ( int i = 0; i < m_primes.size(); ++i )
		{
			while ( ( n % m_primes.get( i ) ) == 0 )
			{
	...
	{
		if ( ( n < 0 ) || ( n > m_high ) ) { throw new IllegalArgumentException( "Not able to handle multiplication by " + " factorial of " + n ); }
		for ( int i = 2; i <= n; ++i )
		{
			multiplyBy( i );
		}
	...
	{
		if ( ( n < 0 ) || ( n > m_high ) ) { throw new IllegalArgumentException( "Not able to handle division by factorial of " + n ); }
		for ( int i = 2; i <= n; ++i )
		{
			divideBy( i );
		}
	...
	public int value()
	{
		...
		for ( int i = 0; i < m_primes.size(); ++i )
		{
			if ( m_values[ i ] < 0 ) { throw new IllegalArgumentException( "Result is not an integer" ); }
			for ( int j = 0; j < m_values[ i ]; ++j )
			{
				final int oldResult = result;
				result *= m_primes.get( i );
				if ( ( result / m_primes.get( i ) ) != oldResult ) { throw new IllegalArgumentException( "Overflow while computing factorial!" ); }
			}
		}
		return result;
	}
