 ...
 * peak suppression. Peaks are found by taking the sign of the difference
 ...
 * it is accurate only for strict peaks, that is, peaks that have no neighbors
 * of equal value.
 * <p>
 ...
 *         modified by Jean-Yves Tinevez to allow for border peaks to be found.
 */
public class PickImagePeaks< T extends RealType< T >> implements OutputAlgorithm< Img< BitType >>, Benchmark
{
	...
	/**
	 * If true, peak will be allowed to found at the image top boundaries. False
	 * by default.
	 */
	private boolean allowBorderPeak = false;

	public PickImagePeaks( final RandomAccessibleInterval< T > inputImage )
	{
		image = inputImage;
		pTime = 0;
	...
	/**
	 * Carries out ellipsoidal peak suppression. This works by first sorting the
	 * peaks in peakList by their corresponding magnitudes, clearing peakList,
	 * then adding the peaks back in, one-by-one, only if they are not within
	 * the suppression ellipsoid of any other peaks that have already been
	 * added, as defined by suppressAxis.
	 */
	private void doSuppression()
	{
		/*
		 * I have a great suspicion that this code, as I write it, will be
		 * fairly slow. Here there be type casts (of ints, doubles, and such),
	...
	public boolean process()
	{
		...
		final Cursor< T > cursor = Views.iterable( image ).localizingCursor();
		 ...
		 * input image. "Sort of" because 1 corresponds to greater than or equal
		 * to zero, while 0 corresponds to less than 0, rather than the
		 * traditions signum. I've written this method in this way in order that
		 * we don't have to care what order the cursor traverses the Img.
		 */
		Img< BitType > imImage;
		T t0, tc;
		...
		// imImagePushCursor is kind of a misnomer. it'll be used for pulling,
		// too, later.
		...
		for ( int d = 0; d < pos.length; ++d )
		{
			...
			while ( imImagePullCursor.hasNext() )
			{
				...
				else if ( d == 0 || peakImageCursor.get().get() )
				{
					...
					/*
					 * (d == 0 || peakImageCursor.getType().get(): If d == 0,
					 * peakImage is assumed to be full of garbage. Otherwise, we
					 * only want to change the value there if it currently true
					 */
	 ...
	 * method. This ArrayList will be populated if it is not already.
	 * 
	 * In the case that there is no peak suppression, the locations are placed
	 * in the list as they are returned by calling the
	 * LocalizableCursor.getPosition() method, and are not guaranteed to have
	 * anything like a natural order.
	 * 
	 * In the case that there is peak suppression, an additional step is taken.
	 * The peaks are first collected, then sorted by magnitude. These are then
	 * selected in order, and the peaks within a distance defined by the
	 * suppressAxis array are rejected.
	 * 
	 * There is no peak suppression if the sum of all values in suppressAxis is
	 * less than 1.
	 * 
	 ...
	 * 
	 * @param r
	 *            the radius of the spheroid of peak suppression.
	 */
	...
	/**
	 * Set the behavior for finding peak at the image boundaries. If set to
	 * false (the default), peaks will not be found at the top borders of the
	 * image.
	 */
	 ...
	 * 
	 * @param r
	 *            an array with as many elements as there are dimensions in the
	 *            {@link Img} with which this {@link PickImagePeaks} was
	 *            created, and represents the extent of the ellipsoid
	 */
	public void setSuppression( final double[] r )
	{
	...
	private class Peak implements Comparable< Peak >
	{
		...
		public int compareTo( final Peak inPeak )
		{
			 ...
			 * Collections.sort() sorts only in the forward direction. I want
			 * these to be sorted in the descending order, and the
			 * Collections.sort method is the only thing that should ever touch
			 * Peak.compareTo. This is faster than sorting, then reversing.
			 */
