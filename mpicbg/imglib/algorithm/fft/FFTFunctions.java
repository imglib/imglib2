			...
			for (int ithread = 0; ithread < threads.length; ++ithread)
				threads[ithread] = new Thread(new Runnable()
				{
					public void run()
					{
						...
						final ImgRandomAccess<S> cursor = complex.createPositionableRasterSampler(); 
		...
		for (int ithread = 0; ithread < threads.length; ++ithread)
			threads[ithread] = new Thread(new Runnable()
			{
				public void run()
				{
					...
					final ImgRandomAccess<S> cursor = complex.createPositionableRasterSampler(); 
					final ImgRandomAccess<T> cursorOut = realImage.createPositionableRasterSampler(); 
...
A:						while( cursorDim.hasNext() )
		...
		for (int ithread = 0; ithread < threads.length; ++ithread)
			threads[ithread] = new Thread(new Runnable()
			{
				public void run()
				{
					...
					final ImgRandomAccess<T> cursor = img.createPositionableRasterSampler( outOfBoundsFactory );
					final ImgRandomAccess<S> cursorOut = fftImage.createPositionableRasterSampler(); 
			...
			for (int ithread = 0; ithread < threads.length; ++ithread)
				threads[ithread] = new Thread(new Runnable()
				{
					public void run()
					{
						...
						final ImgRandomAccess<S> cursor = fftImage.createPositionableRasterSampler(); 
	...
	final private static <T extends Type<T>> void rearrangeQuadrantFFTDimZeroSingleDim( final Image<T> fftImage )
	{
		...
		final ImgRandomAccess<T> cursor1 = fftImage.createPositionableRasterSampler(); 
		final ImgRandomAccess<T> cursor2 = fftImage.createPositionableRasterSampler(); 
