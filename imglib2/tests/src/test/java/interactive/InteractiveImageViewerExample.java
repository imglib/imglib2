...
public class InteractiveImageViewerExample
{
	...
	public static < T extends RealType< T > & NativeType< T > > void show( final RandomAccessibleInterval< T > interval, final T min, final T max, final long width, final long height )
	{
		...
		if ( n == 2 )
		{
			...
			new InteractiveViewer2D< T >( ( int ) width, ( int ) height, source, initial, converter );
		}
		else if ( n == 3 )
		{
			double yScale = 1;
			...
			final InteractiveViewer3D< T > viewer = new InteractiveViewer3D< T >( ( int ) width, ( int ) height, source, interval, converter );
			viewer.setSourceTransform( initial );
		}
		else
		{
