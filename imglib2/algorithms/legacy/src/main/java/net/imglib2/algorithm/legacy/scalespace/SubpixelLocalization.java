/*
 * #%L
 * ImgLib: a general-purpose, multidimensional Img processing library.
 * %%
...
public class SubpixelLocalization< T extends RealType<T> > implements Algorithm, Benchmark, MultiThreaded
{
	...
	public void setLaPlaceImg( final Img<T> laPlacian ) { this.laPlacian = laPlacian; }
	...
	public boolean analyzePeak( final DifferenceOfGaussianPeak<T> peak )
	{
		...
		Img<DoubleType> derivativeVector = doubleArrayFactory.create( new int[] { cursor.numDimensions() }, new DoubleType() );
		...
		if ( !pointsValid )
			return handleFailure( peak, "Moved outside of the Img." );
	 ...
	 */
	protected Matrix invertMatrix( final Img<DoubleType> matrixImg )
	{
		final Matrix matrix = getMatrix( matrixImg );
	...
	{
		final ImgFactory<DoubleType> factory = new ArrayImgFactory<DoubleType>();
		final Img<DoubleType> hessianMatrix = factory.create( new int[] { cursor.numDimensions(), cursor.numDimensions() }, new DoubleType() );
