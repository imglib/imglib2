...
/**
 * This class implements a {@link IterableInterval} representing the volume
 * of a 3D ellipsoid. 
 * <p>
 * The semi-radiuses of the ellipsoid are set by the first 3 elements of the 
 * {@link AbstractNeighborhood#span} array. They are such that the bounding box of 
 * the ellipsoid are <code>2 x span[d] + 1</code> for dimension <code>d</code>.
 * <p>
 * The ellipsoid can be positioned anywhere in a nD image (n >= 3), but will always by 3D.
 * Consequently, only the first 3 elements of the {@link AbstractNeighborhood#span}
...
public class EllipsoidNeighborhood<T> extends AbstractNeighborhood<T> {
	...
	public long size() {
		...
		Utils.getXYEllipseBounds( (int) span[largeAxisDim], (int) span[2], local_rys); 

		// Deal with plane Z = 0
		Utils.getXYEllipseBounds((int) span[smallAxisdim], (int) span[largeAxisDim], local_rxs);
		...
		for (int j = 1; j <= nzplanes; j++) {
			...
			Utils.getXYEllipseBounds(Math.round( (float) local_ry * span[smallAxisdim] / span[largeAxisDim]), local_ry, local_rxs); 
