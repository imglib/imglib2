 ...
 * within the volume of a 3D ball, whose center and radius are given by a
 * {@link SphereNeighborhood}. It is made so that if the ball volume is made of
 * N pixels, this cursor will go exactly over N iterations before exhausting. It
 * takes a spatial calibration into account, which may be non-isotropic, so that
 * the region iterated over is a sphere in <b>physical coordinates</b>.
 * <p>
 ...
 * compute ellipse bounds efficiently.
 * 
 * @author Jean-Yves Tinevez (jeanyves.tinevez@gmail.com) - 2012
 */
public final class SphereCursor<T, IN extends RandomAccessibleInterval<T> & Metadata>
		implements RealPositionableNeighborhoodCursor<T>, Bounded {

	protected final double[] calibration;
	/**
	 * A matching {@link EllipsoidNeighborhood} that has the same dimension in
	 * pixels that the desired sphere.
	 */
	...
	protected final SphereNeighborhood<T, IN> neighborhood;
	/**
	...
	public SphereCursor(SphereNeighborhood<T, IN> neighborhood) {
		this.neighborhood = neighborhood;
		this.calibration = neighborhood.calibration;
		this.pixelCenter = new long[neighborhood.numDimensions()];
		this.ellipsoid = new EllipsoidNeighborhood<T, IN>(neighborhood.source,
				neighborhood.outOfBounds);
		reset();
	}
	...
	/**
	 * Return the square distance measured from the center of the sphere to the
	 * current cursor position, in <b>calibrated</b> units.
	 */
	public double getDistanceSquared() {
		double sum = 0;
		for (int i = 0; i < numDimensions(); i++)
			sum += calibration[i] * calibration[i]
					* ellipsoidCursor.position[i] * ellipsoidCursor.position[i];
		return sum;
	}

	/**
	 * Return the current inclination with respect to this sphere center. Will
	 * be in the range [0, π].
	 * <p>
	...
	public double getTheta() {
		return Math.acos(ellipsoidCursor.position[2] * calibration[2]
				/ Math.sqrt(getDistanceSquared()));
	}
	...
	public double getPhi() {
		return Math.atan2(ellipsoidCursor.position[1] * calibration[1],
				ellipsoidCursor.position[0] * calibration[0]);
	}

	@Override
	public SphereCursor<T, IN> copy() {
		return new SphereCursor<T, IN>(neighborhood);
	}

	@Override
	public SphereCursor<T, IN> copyCursor() {
		return copy();
	}
	...
	public void localize(float[] position) {
		for (int d = 0; d < numDimensions(); d++) {
			position[d] = (float) (calibration[d] * ellipsoidCursor
					.getDoublePosition(d));
		}
	}
