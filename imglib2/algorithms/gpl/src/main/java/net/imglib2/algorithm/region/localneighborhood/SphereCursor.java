 ...
 */
public final class SphereCursor<T> extends AbstractNeighborhoodCursor<T> {
	...
	/** We keep a reference to the source sphere neighborhood. */
	protected final SphereNeighborhood<T> sphere;
	...
	public SphereCursor(SphereNeighborhood<T> neighborhood) {
		super(neighborhood);
		this.sphere = neighborhood;
		this.calibration = neighborhood.calibration;
		this.pixelCenter = new long[neighborhood.numDimensions()];
		this.ellipsoid = new EllipsoidNeighborhood<T, ImgPlus<T>>(neighborhood.source, neighborhood.outOfBounds);
		reset();
	}
	...
	public SphereCursor<T> copy() {
		return new SphereCursor<T>(sphere);
	}
