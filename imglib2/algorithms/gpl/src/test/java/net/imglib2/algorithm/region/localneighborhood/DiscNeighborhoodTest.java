...
public class DiscNeighborhoodTest {
	 ...
	 * </ul>
	 * There is be imprecision on the disc border due to the pixel discretization.
	 */
	@Test
	public final void testBehavior() {
		...
		while(ic.hasNext()) {
			...
			} else if (dist2 >  (radius+calibration[0]) * (radius+calibration[1]) ) {
	...
	public void testSize() {
		...
		for (int i = 0; i < NTRIALS; i++) {
			...
			double[] calibration = new double[] { 0.1 + Math.random(), 0.1 + Math.random() };
			...
			long count = 0;
			for ( @SuppressWarnings("unused") UnsignedByteType type : disc) {
				count++;
			}
