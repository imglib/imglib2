...
public class RectangleNeighborhoodTest {
	...
	public final void testBehavior() {
		...
		Img<UnsignedByteType> image = imgFactory.create(new int[] { DIM, DIM,
				DIM }, new UnsignedByteType());

		long[] center = new long[] { 50, 50, 50 }; // the middle
		long[] span = new long[] { 30, 30, 0 }; // a single plane in the middle

		// Write into the image
		RectangleNeighborhood<UnsignedByteType, Img<UnsignedByteType>> rectangle = new RectangleNeighborhood<UnsignedByteType, Img<UnsignedByteType>>(
				image);
		...
		while (ic.hasNext()) {
			...
			for (int i = 0; i < position.length; i++) {
				if (position[i] < center[i] - span[i]
						|| position[i] > center[i] + span[i]) {
					inside = false;
					break;
				}
			}
