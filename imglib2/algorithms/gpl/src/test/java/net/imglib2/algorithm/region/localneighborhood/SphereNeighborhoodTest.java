...
public class SphereNeighborhoodTest {
	 ...
	 * </ul>
	 * There is be imprecision on the sphere border due to the pixel
	 * discretization.
	 */
	@Test
	public final void testBehavior() {
		...
		Img<UnsignedByteType> img = imgFactory.create(
				new int[] { DIM, DIM, DIM }, new UnsignedByteType());
		ImgPlus<UnsignedByteType> image = new ImgPlus<UnsignedByteType>(img,
				"Radius " + radius, axes, calibration);

		// Write into the image
		SphereNeighborhood<UnsignedByteType, ImgPlus<UnsignedByteType>> sphere = new SphereNeighborhood<UnsignedByteType, ImgPlus<UnsignedByteType>>(
				image, radius);
		...
		while (ic.hasNext()) {
			...
			double dist2 = calibration[0] * (position[0] - center[0])
					...
					* calibration[1] * (position[1] - center[1])
					+ calibration[2] * (position[2] - center[2])
					* calibration[2] * (position[2] - center[2]);
			if (dist2 <= radius * radius) {

				assertEquals(VAL, ic.get().get());

			} else if (Math.sqrt(dist2) > (radius + Math.max(calibration[0],
					Math.max(calibration[1], calibration[2])))) {
