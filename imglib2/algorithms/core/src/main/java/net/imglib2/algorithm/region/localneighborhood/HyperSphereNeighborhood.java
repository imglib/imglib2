 ...
 */
public class HyperSphereNeighborhood<T> extends AbstractLocalizable implements
		Neighborhood<T> {
	public static <T> HyperSphereNeighborhoodFactory<T> factory() {
		return new HyperSphereNeighborhoodFactory<T>() {
			...
			public Neighborhood<T> create(final long[] position,
					final long radius, final RandomAccess<T> sourceRandomAccess) {
				return new HyperSphereNeighborhood<T>(position, radius,
						sourceRandomAccess);
			}
		};
	}
	...
	HyperSphereNeighborhood(final long[] position, final long radius,
	...
	public final class LocalCursor extends AbstractEuclideanSpace implements
			Cursor<T> {
