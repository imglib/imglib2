...
public class OpenAndDisplayInterpolated {
	...
	private void test() {
		...
		final XYProjector< FloatType, ARGBType> projector =
				new XYProjector< FloatType, ARGBType>(interpolatedImg, screenImage, new RealARGBConverter< FloatType>(0, 255));
...
//		imp.show();
		...
		NLinearInterpolation<FloatType> interp = new NLinearInterpolation<FloatType>(img, interpolatedImg);
		interp.update(offset, scale);
...
//		}
	...
	final static public void main(final String[] args) {
		//new ImageJ();
		new OpenAndDisplayInterpolated().test();
	}

	class NLinearInterpolation<T extends NumericType<T>> {
		...
		public <T extends NumericType< T>> NLinearInterpolation(RandomAccessible< T> from,
				IterableInterval< T> to) {
		}

		public void update(double[] offset, double scale) {
			this.offset = offset;
			this.scale = scale;
		}

		public <T extends NumericType< T>> void copyInterpolatedGeneric(
				RandomAccessible< T> from,
				IterableInterval< T> to,
				double[] offset,
				double scale,
