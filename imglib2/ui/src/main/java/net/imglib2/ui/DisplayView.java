...
public class DisplayView<T extends RealType<T> & NativeType<T>> {
	...
	public ArrayList<Converter<T, ARGBType>> getConverters() {
		return converters;
	}
	...
	public DisplayView(final String name, final ImgPlus<T> img,
		...
		if (channelDimIndex < 0) {
			if (luts != null) {
				projector = new XYProjector<T, ARGBType>(img, screenImage, 
					new RealLUTConverter<T>(min, max, luts.get(0)));
			} else {
				projector = new XYProjector<T, ARGBType>(img, screenImage, 
					new RealARGBConverter<T>(min, max));
			}
		} else {
